(function ()
{
    ...
    /** @ngInject */
    function MsNavigationNodeController($scope, $element, $rootScope, $animate, $state, $window, msNavigationService, projectsService)
    {
        var vm = this;
        ...
        vm.window = $window;
        vm.pin_limit_reached = projectsService.pinLimitReached();
        ...
        /**
         * Pin item - always first on the list
         */
        function pinItem(item, event) {
            // pin item
            if(!item.pinned) {

                if(!vm.pin_limit_reached) {
                    event.stopPropagation();
                    event.preventDefault();

                    item.pinned = true;
                    // update navigation - save data to localStorage
                    projectsService.setPinnedStatus(item, true);
                }
            // unpin item
            } else {
                event.stopPropagation();
                ...
                item.pinned = false;
                projectsService.setPinnedStatus(item, false);
            }   
        }
